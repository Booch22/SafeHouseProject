...
def marker( select_cam : int ) :
    ...
    # นำเข้ารูปภาพ
    image = cv2.imread( "Benchmark_Normal_Frames/frame288.jpg" ) # cam 3 = 436
...
ruler_mill, ruler_pix = marker( 1 )
print( f"{ruler_mill}, {ruler_pix}" )


# กำหนดค่า threshold
a_threshold = 134 # เขียว 134-136
b_threshold = 120 # น้ำเงิน
...
for frame in range( 1500 ) :
    ...
    sub_image_1 = input_image[region_1[1]:region_1[1]+region_1[3], region_1[0]:region_1[0]+region_1[2]]
    ...
    gauss_blur_a_img = cv2.GaussianBlur( a_img, (9, 9), 10 )
    gauss_blur_b_img = cv2.GaussianBlur( b_img, (9, 9), 10 )


    # ทำการ thresholding
    ret_a, thresh_a_img = cv2.threshold( gauss_blur_a_img, a_threshold, 255, cv2.THRESH_BINARY_INV )
    ...
    # morphology ดำ
    opening_a_img = cv2.morphologyEx( thresh_a_img, cv2.MORPH_OPEN, kernel )
    closing_a_img = cv2.morphologyEx( opening_a_img, cv2.MORPH_CLOSE, kernel )
    ...
    size_threshold( closing_a_img, 250, 450 ) # นับวัตถุที่เป็นสีขาว
    size_threshold( closing_b_img, 150, 400 ) # นับวัตถุที่เป็นสีขาว


    # ตรวจความกลมของวัตถุ หากวัตถุไม่กลมให้ถมดำ
    check_circularity( closing_a_img )
    #check_circularity( closing_b_img )

    # ตรวจจับหมุดและเหรียญ
    centr_x_green, centr_y_green = detect_object( main_img, closing_a_img )
    ...
    calculate_distance( main_img, centr_x_green, centr_y_green, ruler_mill, ruler_pix )

    
    # แสดงภาพที่ได้จาก Image Thresholding
    cv2.imshow( "main image", main_img )
    cv2.imshow( "k image", closing_b_img )
    cv2.waitKey(50) # 220
    start_frame += 2 
